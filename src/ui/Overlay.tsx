...
import { buildVibelaZip, triggerZipDownload } from '../shared/downloadExport';
...
function newAnnotationId(prefix = 'annotation'): string {
  return crypto.randomUUID?.() ?? `${prefix}-${Date.now()}-${Math.random().toString(16).slice(2) || '0'}`;
}
...
export function Overlay({ active, allowed, shadowHostEl, tabId }: OverlayProps) {
  ...
  // Firefox (and any browser without the File System Access API) cannot write
  // .vibela/ to disk. Instead we package the same payload into a ZIP the user
  // unzips at their repo root, keeping the browser-to-code loop usable there.
  function handleExportZip() {
    if (annotations.length === 0) {
      setStatus('nada que exportar — primero anota algo');
      return;
    }
    try {
      const result = buildVibelaZip(annotations, { pathname: window.location.pathname });
      triggerZipDownload(result.bytes, 'vibela-export.zip');
      const msg = `Exportado · ${result.count} anotación${result.count === 1 ? '' : 'es'} → vibela-export.zip (descomprimí en la raíz del repo)`;
      setStatus(msg);
      window.setTimeout(() => setStatus((s) => (s === msg ? '' : s)), 4000);
    } catch (err) {
      setStatus(err instanceof Error ? err.message : 'falló la exportación');
    }
  }
          ...
          <div className="vc-panel-body">
            ...
            <div className="vc-actions">
              ...
              {connection === 'unsupported' && (
                <button
                  type="button"
                  data-testid="vc-btn-export-zip"
                  title="Este navegador no puede escribir al disco. Exporta un .zip para descomprimir en tu repo."
