 ...
 * Returns the directory handle's name.
 */
...
async function scaffoldVibela(
  ...
  // Screenshots capture whatever is on screen and may hold sensitive data, so
  // keep them out of version control by default. Written only when absent so a
  // user who deliberately tracks them is never overridden.
  await writeTextFileIfAbsent(
    vibelaDir,
    '.gitignore',
    '# Vibela screenshots can capture sensitive on-screen content — keep them local.\nscreenshots/\n',
  );
...
export async function sync(
  ...
  // Validate ids ONCE here so incomingTasks and the screenshot writes derive from
  // the same set — an unsafe id can never yield a task whose screenshotPath points
  // at a file the screenshot loop refused to write.
  const safeAnnotations = filterSafeAnnotations(annotations);

  const incomingTasks: VibelaTask[] = safeAnnotations.map((a, i) => {
