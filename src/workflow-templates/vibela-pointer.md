...
- **Queue:** `.vibela/tasks.json` (source of truth) mirrored by `.vibela/tasks.md`
  — keep both in sync. Screenshots live at `.vibela/screenshots/<id>.png`
  (transform tasks use `<id>-before.png` / `<id>-after.png`), a full-viewport
  shot with the annotated element highlighted. Only process tasks whose `status`
  is `"to do"`. `stream.jsonl` is an extension-owned append-only log — never
  modify it.
- **Lifecycle (no shortcuts):** `to do → doing → done`, or `doing → failed → to do`.
  ...
  the MD in the same batch. Forbidden: `to do → done`, `done → doing`,
  `done → failed`, `failed → done`.
- **Mode:** default to step (one task at a time, pause for approval). Use batch for
  6+ same-type tasks targeting the same component or file; yolo only when
  explicitly asked.
